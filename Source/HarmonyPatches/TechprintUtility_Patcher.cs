...
public static class TechprintUtility_Patcher
{
    ...
    public static bool TryGetTechprintDefToGenerate_Postfix(bool __state, Faction faction, out ThingDef result, List<ThingDef> alreadyGeneratedTechprints, float maxMarketValue)
    {
        ...
        if (!TechprintUtility.GetResearchProjectsNeedingTechprintsNow(faction, alreadyGeneratedTechprints, maxMarketValue)
                             .TryRandomElementByWeight(def => GetFactionSpecificSelectionWeight(def, faction), out ResearchProjectDef projectDef))
        {
            result = null;
            return false;
        }
    ...
    public static float GetFactionSpecificSelectionWeight(ResearchProjectDef project, Faction faction = null)
    {
        ...
        float ifNativeModifier = project.HasModExtension<NativeTechprint_DefModExtension>() ? ConfigurableTechprintsMod.Instance.Settings.NativeCommonalityMultiplier : 1f;
