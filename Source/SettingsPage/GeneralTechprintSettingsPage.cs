...
using UnityEngine;
using Verse;
using TechLevel = RimWorld.TechLevel;
...
namespace ConfigurableTechprints.SettingsPage
{
    internal class GeneralTechprintSettingsPage : ConfigurableTechprintsSettingPage
    {
        protected override void DoPage(Listing_Standard list, Rect inRect)
        {
            ...
            list.SliderWithTextField(ref settings.TechprintPerResearchPoints, ref tprStringBuffer, 100, 24000, 50,
                                     "TechprintPerResearchPoints_Tooltip".Translate());

            list.Gap();
            list.Label("MarketPriceMultiplier_Label".Translate());
            list.SliderWithTextField(ref settings.MarketPriceMultiplier, ref mpmStringBuffer, 0.01f, 10f,
                                     tooltip: "MarketPriceMultiplier_Tooltip".Translate());

            list.Gap();
            list.Label("BaseCommonality_Label".Translate());
            list.SliderWithTextField(ref settings.BaseCommonality, ref bcStringBuffer, 0.01f, 100f,
                                     tooltip: "BaseCommonality_Tooltip".Translate());

            list.Gap();
            list.Label($"<b>{"ProjectModifications_Heading".Translate()}</b>");
            list.CheckboxLabeled("ModifyBaseCosts_Label".Translate(), ref settings.ModifyBaseCosts,
                                 "ModifyBaseCosts_Tooltip".Translate());
            ...
            list.SliderWithTextField(ref settings.ResearchBaseCostMultiplier, ref rbcStringBuffer, 0.01f, 10f,
                                     tooltip: "ResearchBaseCostMultiplier_Tooltip".Translate());
        }
    }
}
