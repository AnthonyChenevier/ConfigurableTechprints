...
namespace ConfigurableTechprints.DefProcessors
{
    public static class TraderKindDefProcessor
    {
        ...
        public static void SetCountChances(this StockGenerator_Techprints stockGenerator, List<CountChance> countChances) => Traverse.Create(stockGenerator).Field("countChances").SetValue(countChances);
        public static List<CountChance> GetCountChances(this StockGenerator_Techprints stockGenerator) => Traverse.Create(stockGenerator).Field("countChances").GetValue<List<CountChance>>();
        ...
        public static string ProcessTechprintTrader(TraderKindDef trader, float traderStockCountMultiplier)
        {
            ...
            countChances = countChances.Select(cc => new CountChance { chance = cc.chance, count = Mathf.CeilToInt(cc.count * traderStockCountMultiplier) }).ToList();
